...
class Device:
    ...
    def __str__(self):
        if self.name:
            return f"{self.name} ({self.ip}, {self.mac}, {self.vendor}, {self.uuid})"
        return f"({self.ip}, {self.mac}, {self.vendor}, {self.uuid})"

    def __repr__(self):
        if self.name:
            return f"Device(ip='{self.ip}', mac='{self.mac}', vendor='{self.vendor}', name='{self.name}', uuid='{self.uuid}')"
        return f"Device(ip='{self.ip}', mac='{self.mac}', vendor='{self.vendor}', uuid='{self.uuid}')"
